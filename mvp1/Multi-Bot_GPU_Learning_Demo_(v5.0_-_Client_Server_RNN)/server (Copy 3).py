...
class GridEnvironment:
    ...
    def randomize(self):
        ...
        # No density check or warning here anymore
        ...
        # --- Helper to get the next available position ---
        def get_next_available_position(occupied_set: Set) -> Optional[Dict]:
            if not available_cells: # No more cells left on the grid
                return None
            x, y = available_cells.pop()
            pos_str = f"{x},{y}"
            # In the extremely unlikely event a shuffled cell was already somehow occupied (shouldn't happen with this logic), try the next
            while pos_str in occupied_set and available_cells:
                 x, y = available_cells.pop()
                 pos_str = f"{x},{y}"
            if pos_str in occupied_set: # If still occupied after exhausting list
                 return None
            occupied_set.add(pos_str)
            return {'x': x, 'y': y}

        # --- Place Goals (Unconstrained by distance) ---
        goal_id_counter = 0
        placed_goals = 0
        for _ in range(self.num_goals):
            pos = get_next_available_position(occupied)
            if pos is None:
                print(f"Warning: Grid full. Placed only {placed_goals}/{self.num_goals} goals.")
                break
            goal = {**pos, 'id': f"G{goal_id_counter}"}; goal_id_counter += 1
            self.goals.append(goal); placed_goals += 1

        # --- Place Bots (Unconstrained by distance) ---
        placed_bots = 0
        bots_to_place = [(f"H{i}", "Hardcoded") for i in range(self.num_hc_bots)] + \
                        [(f"L{i}", "Learning") for i in range(self.num_learning_bots)]
        random.shuffle(bots_to_place) # Shuffle bot types/ids for placement order

        for bot_id, bot_type in bots_to_place:
             pos = get_next_available_position(occupied)
             if pos is None:
                  print(f"Warning: Grid full. Placed only {placed_bots}/{total_bots} bots.")
                  break
             self.start_positions.append({**pos, 'type': bot_type, 'id': bot_id}); placed_bots += 1

        # Ensure start_positions are correctly ordered if needed elsewhere (e.g., bot creation loop)
        # This sorting isn't strictly necessary with the current setup_simulation logic, but good practice.
        self.start_positions.sort(key=lambda sp: (sp['type'], int(sp['id'][1:])))


        # --- Place Obstacles (Using remaining available cells) ---
        num_obstacles_to_place = random.randint(self.min_obstacles, self.max_obstacles);
        placed_obstacles = 0
        # Place obstacles in remaining available cells
        for _ in range(num_obstacles_to_place):
            pos = get_next_available_position(occupied) # Takes from remaining shuffled cells
            if pos is None:
                # This just means we ran out of cells before placing the desired number of obstacles
                # print(f"Note: Grid full before placing all {num_obstacles_to_place} requested obstacles. Placed {placed_obstacles}.")
                break
            # No need to add to 'occupied' here as get_next_available_position already did
            self.obstacles.add(self._pos_to_string(pos))
            placed_obstacles += 1

        # Report final placement numbers
...
# --- Main Simulation Loop Task (Corrected End-of-Round Logic) ---
async def simulation_loop():
    global is_running, environment, all_bots, hc_total_goals, learning_total_goals
    print("Simulation loop started.")
    step_counter = 0
    error_occurred = False # Track errors across the loop lifetime

    while is_running:
        start_time = time.monotonic()
        round_over_signal = None
        current_step_error = False # Track error within this specific step
        bots_state_update = []
        goals_claimed_this_step = False
        step_losses = []

        # --- Lock for the main processing of one step ---
        async with _simulation_lock:
            # Check components again inside lock, in case they were removed between loop iterations
            if not is_running or not environment or not all_bots or not cortical_library:
                 print("Simulation loop stopping mid-step: Components missing.");
                 is_running = False # Ensure state is consistent
                 error_occurred = True # Mark as error state
            ...
            for bot in all_bots:
                 if bot.steps >= CONFIG['MAX_STEPS_PER_ROUND']: continue # Skip bots that finished max steps
                 ...
                 try:
                    ...
                    if isinstance(bot, HardcodedBot): action = bot.get_action(senses, environment)
                    elif isinstance(bot, LearningBot): action = bot.get_action(senses, environment)
                    else: action = -1
                    ...
                    current_step_error = True; error_occurred = True; break # Break inner loop

            if current_step_error: # If error occurred getting actions, stop processing this step
                 is_running = False; break # Also stop the outer loop
            ...
            for bot in all_bots:
                ...
                next_p = current_pos
                if action == -1 or bot.freeze_timer > 0: pass
                elif 0 <= action <= 3:
            ...
            if current_step_error: # Stop loop if learning step had error
                is_running = False; break

            # --- Send Updates (Inside Lock still) ---
            ...
            if step_counter % 5 == 0 or goals_claimed_this_step:
                 await send_stats_update()
                 await send_library_state_update()

            # --- Check Round End Conditions (Still inside Lock, signals only) ---
            if not round_over_signal: # Check again if not already set by goal claim
        ...
        # --- Handle Round End and Reset (Outside the step lock) ---
        if round_over_signal:
            ...
            if is_running: # Check if we should auto-start next round
                try:
                    # Call reset function (which will acquire lock itself)
                    if not await reset_round_state_internal(randomize_env=True):
                        # Reset failed, stop the simulation
                        print("Reset round failed. Stopping simulation.")
                        await send_status_update("Error: Failed to reset round. Stopped.", is_error=True)
                        is_running = False # Signal loop to stop
                        # No break here, loop condition will handle exit
                    else:
                        # Reset succeeded, reset step counter for the new round
                        step_counter = 0
                        # Status update is handled within reset_round_state_internal
                except Exception as reset_error:
                    print(f"Error auto-resetting round: {reset_error}"); traceback.print_exc()
                    await send_status_update("Error resetting round! Stopped.", is_error=True)
                    is_running = False # Signal loop to stop
                    # No break here, loop condition handles exit
            else:
                 # If is_running was already false (e.g., user clicked Stop), just report round end
                 await send_status_update(f"Round {round_number} Finished ({reason}). Stopped.")
                 # No break needed, loop condition handles exit

        # --- Delay ---
        # Calculate delay only if we are still running
        if is_running:
            ...
            # Use asyncio.sleep outside the lock to avoid holding it during sleep
            await asyncio.sleep(delay_s)

    # --- Loop End ---
    print("Simulation loop finished.")
    # Send final state updates outside the loop
    try:
        ...
        print(f"Error sending final updates: {final_update_e}")

    # Send final status update ONLY if no error caused the exit
    # If an error occurred, the status was likely already set to error/stopped
    if not error_occurred:
        await send_status_update("Status: Stopped.")
    # else: the error message should persist
